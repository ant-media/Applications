...
export class WebRTCAdaptor {
      ...
      this.mediaManager.navigatorUserMedia(
        ...
        (stream) => {
          ...
          var jsCmd = {
            ...
            mainTrack: typeof mainTrack !== undefined ? mainTrack : "",
            video:
              this.mediaManager.localStream.getVideoTracks().length > 0
                ? true
                : false,
            audio:
              this.mediaManager.localStream.getAudioTracks().length > 0
                ? true
                : false,
            metaData: metaData,
          };
  ...
  /**
   * function used for start/stop custom music
   * streamId: unique id of the stream belogns to this participant
   * newStream: media stream to pass
   * status: it is use as identifier for start/stop/mix music
   */
  prepareNewMusicStreamTracks(streamId, newStream, status) {
    console.log("Inside prepare stream track");
    this.turnOnLocalCamera(streamId);
    if (status == "startAudio") {
      //Calling updateAudioTrack for updating audio with music file audio stream
      var mediaConstraints = {
        video: true,
        audio: "startAudio",
      };
      this.mediaManager.prepareStreamTracks(
        mediaConstraints,
        "startAudio",
        newStream,
        streamId
      );
    } else if (status == "stopAudio") {
      var mediaConstraints = {
        video: true,
        audio: "stopAudio",
      };
      //Calling prepareStreamTracks for updating audio with system audio stream
      this.mediaManager.prepareStreamTracks(
        mediaConstraints,
        "stopAudio",
        newStream,
    ...
    } else if (status == "mixAudio") {
      var mediaConstraints = {
        video: true,
        audio: true,
      };
      //Calling prepareStreamTracks for mixing background audio with system audio stream
      this.mediaManager.prepareStreamTracks(
        mediaConstraints,
        "mixAudio",
        newStream,
    ...
    if (this.remotePeerConnection[streamId] == null) {
      ...
      if (!this.playStreamId.includes(streamId)) {
        if (this.mediaManager.localStream != null) {
          this.remotePeerConnection[streamId].addStream(
      ...
      this.remotePeerConnection[streamId].oniceconnectionstatechange = (
        ...
        if (!this.isPlayMode && !this.playStreamId.includes(streamId)) {
          if (
            ...
            this.mediaManager
              .changeBandwidth(this.mediaManager.bandwidth, streamId)
              .then(() => {
                console.log(
                  "Bandwidth is changed to " + this.mediaManager.bandwidth
                );
              })
  ...
  getSoundLevelList(streamsList) {
    for (let i = 0; i < streamsList.length; i++) {
      this.soundLevelList[streamsList[i]] =
        this.soundMeters[streamsList[i]].instant.toFixed(2);
    }
  ...
  getSender(streamId, type) {
    var sender = null;
    if (this.remotePeerConnection[streamId] != null) {
      sender = this.remotePeerConnection[streamId]
        .getSenders()
        .find(function (s) {
  ...
  turnOnLocalCamera(streamId) {
    this.mediaManager.turnOnLocalCamera(streamId);
  }
  muteLocalMic() {
