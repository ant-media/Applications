...
export class MediaManager {
   ...
   */
  initLocalStream() {
    console.log("media_manager initLocalStream");
        ...
        (audioStream) => {
          ...
          if (this.publishMode == "screen") {
            ...
            if (
              ...
              audioConstraint != undefined &&
              audioConstraint != "startAudio" &&
              audioConstraint != "stopAudio" &&
              audioConstraint != "mixAudio"
            ) {
              stream.addTrack(audioStream.getAudioTracks()[0]);
            } else if (audioConstraint == "mixAudio") {
              console.log("Start Mixing Music");
              const bgAudio = document.getElementById("myaudio");
              bgAudio.play();
              const astream = bgAudio.captureStream();
              stream.addTrack(audioStream.getAudioTracks()[0]);
              var str = this.mixAudioStreams(stream, astream);
              this.updateAudioTrack(str, streamId, null);
            } else if (audioConstraint == "startAudio") {
              console.log("Start Music");
              const bgAudio = document.getElementById("myaudio");
              bgAudio.play();
              const astream = bgAudio.captureStream();
              this.updateAudioTrack(astream, streamId, null);
            } else if (audioConstraint == "stopAudio") {
              console.log("Stoping Music");
              stream.addTrack(audioStream.getAudioTracks()[0]);
              this.updateAudioTrack(stream, streamId, null);
            }
            this.gotStream(stream);
          }
        },
